...
void BTNPORT_Handler(uint8_t num){
    ...
    if (num == KEY_VALUE_2)
    {
        ...
        {
            printf("master send......\n");
            LIN_DRV_MasterSendHeader(INST_LIN0, FRAME_SLAVE_RECEIVE_DATA);
        }
        else
        {
            printf("master receive......\n");
            LIN_DRV_MasterSendHeader(INST_LIN0, FRAME_MASTER_RECEIVE_DATA);
        }
    ...
    if (num == KEY_VALUE_3)
    {
        ...
        {
            printf("%s send sleep signal.\n", print_role_str());
            /* Send header to set the node to sleep mode */
            LIN_DRV_MasterSendHeader(INST_LIN0, FRAME_GO_TO_SLEEP);
        }
    }
}
    ...
    switch (lin0_State->currentEventId)
    {
        ...
        case LIN_PID_OK:
            ...
            if(FRAME_SLAVE_RECEIVE_DATA == lin0_State->currentId)
            {
                ...
                LIN_DRV_SendFrameData(INST_LIN0, txBuff1, sizeof(txBuff1));
            }
...
static void LinMsg_WorkingHandler(void)
{
    ...
    else if (FRAME_MASTER_RECEIVE_DATA == lin0_State.currentId)
    {
        ...
        if((status == STATUS_SUCCESS) && (0U == byteRemain))
        {
            ...
            if(memcmp(rxBuff, txBuff2, DATA_SIZE) != 0)
            {
                receiveFlag = false;
            }
