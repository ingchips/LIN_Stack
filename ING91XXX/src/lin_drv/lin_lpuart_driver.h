...
extern const Lin_IRQn_Type g_linLpuartRxTxIrqId[LPUART_INSTANCE_COUNT];
...
{
    linCurrentState->currentBreakCharTransmitLength = (uint8_t)length;
    base->Control &= (~(1<<bsUART_ENABLE)); // uart disable.
    switch(length)
    {
        case LPUART_BREAK_CHAR_10_BIT:       //(if start=1, data=8, [parity=0, stop=1])
            base->LineCon_H = (base->LineCon_H & ~(7UL<<bsUART_PARITY_SELECT)) | (0UL << bsUART_PARITY_SELECT); //BIT[3:1] = 0UL
            break;
        case LPUART_BREAK_CHAR_11_BIT:       //(if start=1, data=8, [parity=1, stop=1])
            base->LineCon_H = (base->LineCon_H & ~(7UL<<bsUART_PARITY_SELECT)) | (3UL << bsUART_PARITY_SELECT); //BIT[3:1] = 3UL
            break;
        default: //LPUART_BREAK_CHAR_12_BIT: //(if start=1, data=8, [parity=1, stop=2])
            /* LPUART_BREAK_CHAR_13_BIT, LPUART_BREAK_CHAR_14_BIT, LPUART_BREAK_CHAR_15_BIT will also use 12bits configuration. */
            base->LineCon_H = (base->LineCon_H & ~(7UL<<bsUART_PARITY_SELECT)) | (7UL << bsUART_PARITY_SELECT); //BIT[3:1] = 7UL
            break;
    }
...
static inline void LPUART_SetBreakCharDetectLength( LPUART_Type * base,
                                                    ...
                                                    lpuart_break_char_length_t length)
{
    linCurrentState->currentBreakCharDetectLength = (uint8_t)length;
    base->Control &= (~(1<<bsUART_ENABLE)); // uart disable.
    switch(length)
    {
        case LPUART_BREAK_CHAR_10_BIT:       //(if start=1, data=8, [parity=0, stop=1])
...
{
    base->Control &= (~(1<<bsUART_ENABLE)); // uart disable.
    base->LineCon_H |= (1UL << bsUART_SEND_BREAK);
    base->Control |= (1<<bsUART_ENABLE); // uart enable.
}

static inline void LPUART_DequeueBreakField(LPUART_Type * base)
{
    base->Control &= (~(1<<bsUART_ENABLE)); // uart disable.
    base->LineCon_H &= (~(1UL << bsUART_SEND_BREAK));
    base->Control |= (1<<bsUART_ENABLE); // uart enable.
...
/* Unit: 0.1bit. delayBits = 0.1*bit_step_num. */
static inline void LPUART_DelayBitsByBaudrate(uint32_t baudRate, uint32_t bit_step_num )
{
    if(baudRate && bit_step_num)
    {
        /* Calculate delay time(us). */
        uint32_t delayUs = (bit_step_num * 1000000 / 10 / baudRate);
...
static inline uint32_t get_break_time_us_by_break_bits(uint32_t baudRate, lpuart_break_char_length_t break_bits)
{
    return (break_bits * 1000000 / baudRate);
}
